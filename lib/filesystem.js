  ...
  var ready = function () {
    var get = function (path, cb) {
      ...
      db.get(toIndexKey(path), {valueEncoding: 'json'}, function (err, entry) {
        ...
        fs.stat(entry.layer, function (err, stat) {
          ...
          log('get returning with entry:', JSON.stringify(entry))
          cb(null, entry)
        })
      })
    }
    ...
    handlers.getattr = function (path, cb) {
      ...
      get(path, function (err, entry) {
        ...
        if (entry.type === 'file') {
          ...
          stat.nlink = 1
          if (entry.layer) {
            fs.stat(entry.layer, function (err, fstat) {
              if (err) {
                console.log('fstat error:', err)
                return cb(err)
              }
              stat.nlink = fstat.nlink
              log('getattr - returning after fstat')
              return cb(0, stat)
            })
          } else {
            log('getattr- returning without fstat')
            return cb(0, stat)
          }
        }
    ...
    handlers.readdir = function (path, cb) {
      ...
      collect(rs, function (err, entries) {
        ...
        log('readdir returning with files:', files)
        cb(0, files)
      })
    }
    ...
    handlers.unlink = function (path, cb) {
      ...
      get(path, function (err, entry) {
        ...
        db.del(toIndexKey(path), function () {
          ...
          var list = files[path] || []
          if (!list.length) {
            return fs.unlink(p.join(store, 'layer', entry.layer), function () {
              cb(0)
            })
          }
          return cb(0)
    ...
    handlers.symlink = function (src, dest, cb) {
      ...
      db.put(toIndexKey(dest), { type: 'symlink', linkname: src }, function (err) {
        if (err) return cb(err)
        return cb(0)
      })
    }
